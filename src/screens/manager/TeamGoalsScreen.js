...
export const TeamGoalsScreen = ({ navigation }) => {
  const insets = useSafeAreaInsets();
  const goals = [
    { title: 'Complete Q1 Sprint Goals', progress: 72, deadline: 'Mar 31, 2026', priority: 'high' },
    { title: 'Reduce Bug Count by 20%', progress: 45, deadline: 'Jun 30, 2026', priority: 'medium' },
    { title: 'Launch New Feature', progress: 30, deadline: 'Apr 15, 2026', priority: 'high' },
    { title: 'Improve Code Coverage', progress: 60, deadline: 'May 31, 2026', priority: 'low' },
  ];
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        <View style={styles.statsRow}>
          <StatCard icon="flag" label="Total Goals" value={goals.length} color={colors.primary} delay={0} />
          <StatCard icon="checkmark-circle" label="On Track" value={goals.filter(g => g.progress >= 50).length} color={colors.success} delay={100} />
          <StatCard icon="warning" label="Behind" value={goals.filter(g => g.progress < 30).length} color={colors.warning} delay={200} />
        </View>
...
const styles = StyleSheet.create({
  ...
  goalDeadline: { ...typography.bodySmall, color: colors.textSecondary, marginBottom: spacing.sm },

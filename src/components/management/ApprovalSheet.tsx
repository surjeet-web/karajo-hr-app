...
interface ApprovalSheetProps {
  visible: boolean;
  onClose: () => void;
  onApprove?: (request: ApprovalRequest, comment: string) => void;
  onReject?: (request: ApprovalRequest, comment: string) => void;
  request?: ApprovalRequest;
}

export const ApprovalSheet: React.FC<ApprovalSheetProps> = ({ visible, onClose, onApprove, onReject, request }) => {
  const [comment, setComment] = useState('');

  const handleApprove = () => {
    hapticFeedback('success');
    onApprove?.(request as ApprovalRequest, comment);
    setComment('');
    onClose();
  };
  ...
  const handleReject = () => {
    ...
    onReject?.(request as ApprovalRequest, comment);
    setComment('');
    onClose();
  };
